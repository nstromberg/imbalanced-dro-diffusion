...
def model_wrapper(
    ...
    def get_model_input_time(t_continuous):
        ...
        if noise_schedule.schedule == 'discrete':
            # print('discrete time:', (t_continuous - 1. / noise_schedule.total_N) * 1000.)
    ...
    def model_fn(x, t_continuous):
        ...
        if guidance_type == "uncond":
            noise_prediction, _ = torch.split(noise_pred_fn(x, t_continuous), 3, dim = 1)
            ...
            if guidance_scale == 1. or unconditional_condition is None:
                noise_prediction, _ = torch.split(noise_pred_fn(x, t_continuous, cond=condition), 3, dim = 1)
                return noise_prediction
...
class MultistepSDE:
    ...
    def get_time_steps(self, skip_type, t_T, t_0, N, order, device):
        ...
        if skip_type == 'logSNR':
            ...
            # logSNR_steps = torch.linspace(lambda_T.cpu().item()**(1. / order), lambda_0.cpu().item()**(1. / order), N + 1).pow(order).to(device)
            logSNR_steps = lambda_T + torch.linspace(torch.tensor(0.).cpu().item(), (lambda_0-lambda_T).cpu().item()**(1. / order), N + 1).pow(order).to(device)
            ...
            sigma_max = min(80.204, self.sigma_max)
    ...
    def get_coefficients_exponential_negative(self, order, interval_start, interval_end):
        ...
        if order == 0:
            ...
            return torch.exp(-interval_end) * ((interval_start + 1) * torch.exp(interval_end - interval_start) - (interval_end + 1))
        elif order == 2:
            return torch.exp(-interval_end) * ((interval_start ** 2 + 2 * interval_start + 2) * torch.exp(interval_end - interval_start) - (interval_end ** 2 + 2 * interval_end + 2))
        elif order == 3:
            return torch.exp(-interval_end) * ((interval_start ** 3 + 3 * interval_start ** 2 + 6 * interval_start + 6) * torch.exp(interval_end - interval_start) - (interval_end ** 3 + 3 * interval_end ** 2 + 6 * interval_end + 6))
    ...
    def get_coefficients_exponential_positive(self, order, interval_start, interval_end, tau):
        ...
        if order == 0:
            return torch.exp(interval_end_cov) * (1 - torch.exp(-(interval_end_cov - interval_start_cov))) / ((1 + tau ** 2))
        elif order == 1:
            return torch.exp(interval_end_cov) * ((interval_end_cov - 1) - (interval_start_cov - 1) * torch.exp(-(interval_end_cov - interval_start_cov))) / ((1 + tau ** 2) ** 2)
        elif order == 2:
            return torch.exp(interval_end_cov) * ((interval_end_cov ** 2 - 2 * interval_end_cov + 2) - (interval_start_cov ** 2 - 2 * interval_start_cov + 2) * torch.exp(-(interval_end_cov - interval_start_cov))) / ((1 + tau ** 2) ** 3)
        elif order == 3:
            return torch.exp(interval_end_cov) * ((interval_end_cov ** 3 - 3 * interval_end_cov ** 2 + 6 * interval_end_cov - 6) - (interval_start_cov ** 3 - 3 * interval_start_cov ** 2 + 6 * interval_start_cov - 6) * torch.exp(-(interval_end_cov - interval_start_cov))) / ((1 + tau ** 2) ** 4)
    ...
    def get_coefficients_fn(self, order, interval_start, interval_end, lambda_list, tau):
        ...
        for i in range(order):
            ...
            for j in range(order):
                if self.predict_x0:
                    coefficient += lagrange_coefficient[i][j] * self.get_coefficients_exponential_positive(order - 1 - j, interval_start, interval_end, tau)
                else:
                    coefficient += lagrange_coefficient[i][j] * self.get_coefficients_exponential_negative(order - 1 - j, interval_start, interval_end)
    ...
    def adams_bashforth_update(self, order, x, tau, model_prev_list, t_prev_list, noise, t):
        ...
        gradient_coefficients = self.get_coefficients_fn(order, ns.marginal_lambda(t_prev_list[-1]), lambda_t, lambda_list, tau)

        for i in range(order):
            if self.predict_x0:
                gradient_part += (1 + tau ** 2) * sigma_t * torch.exp(- tau ** 2 * lambda_t) * gradient_coefficients[i] * model_prev_list[-(i+1)]
            else:
    ...
    def adams_bashforth_update_few_steps(self, order, x, tau, model_prev_list, t_prev_list, noise, t):
        ...
        if self.predict_x0:
            if order == 2: ## if order = 2 we do a modification that does not influence the convergence order similar to unipc. Note: This is used only for few steps sampling.
                ...
                gradient_coefficients[0] += 1.0 * torch.exp((1 + tau ** 2) * lambda_t) * (h ** 2 / 2 - (h * (1 + tau ** 2) - 1 + torch.exp((1 + tau ** 2) * (-h))) / ((1 + tau ** 2) ** 2)  ) / (ns.marginal_lambda(t_prev_list[-1]) - ns.marginal_lambda(t_prev_list[-2]))
                gradient_coefficients[1] -= 1.0 * torch.exp((1 + tau ** 2) * lambda_t) * (h ** 2 / 2 - (h * (1 + tau ** 2) - 1 + torch.exp((1 + tau ** 2) * (-h))) / ((1 + tau ** 2) ** 2)  ) / (ns.marginal_lambda(t_prev_list[-1]) - ns.marginal_lambda(t_prev_list[-2]))
    ...
    def sample_few_steps(self, x, tau, steps=5, t_start=None, t_end=None, skip_type='time', skip_order=1, predictor_order=3, corrector_order=4, pc_mode='PEC', return_intermediate=False
    ):
        ...
        with torch.no_grad():
            ...
            timesteps = self.get_time_steps(skip_type=skip_type, t_T=t_T, t_0=t_0, N=steps, order = skip_order, device=device)
            ...
            for step in range(1, max(predictor_order, corrector_order - 1)):
                ...
                noise = torch.randn_like(x)
                # predictor step
                x_p = self.adams_bashforth_update_few_steps(order = predictor_order_used, x = x, tau = tau(t), model_prev_list = model_prev_list, t_prev_list = t_prev_list, noise = noise, t = t)
                # evaluation step
                ...
                # corrector step
                x = self.adams_moulton_update_few_steps(order = corrector_order_used, x = x, tau = tau(t), model_prev_list = model_prev_list, t_prev_list = t_prev_list, noise = noise, t = t)
            ...
            for step in range(max(predictor_order, corrector_order - 1), steps + 1):
                ...
                if skip_final_step and step == steps and not denoise_to_zero:
                    x_p = self.adams_bashforth_update_few_steps(order = predictor_order_used, x = x, tau = 0, model_prev_list = model_prev_list, t_prev_list = t_prev_list, noise = noise, t = t)
                else:
                    x_p = self.adams_bashforth_update_few_steps(order = predictor_order_used, x = x, tau = tau(t), model_prev_list = model_prev_list, t_prev_list = t_prev_list, noise = noise, t = t)
    ...
    def sample_more_steps(self, x, tau, steps=20, t_start=None, t_end=None, skip_type='time', skip_order=1, predictor_order=3, corrector_order=4, pc_mode='PEC', return_intermediate=False
    ):
        ...
        with torch.no_grad():
            ...
            for step in range(1, max(predictor_order, corrector_order - 1)):
                ...
                noise = torch.randn_like(x)
                # predictor step
                x_p = self.adams_bashforth_update(order = predictor_order_used, x = x, tau = tau(t), model_prev_list = model_prev_list, t_prev_list = t_prev_list, noise = noise, t = t)
                # evaluation step
                model_x = self.model_fn(x_p, t)
                ...
                model_prev_list.append(model_x)
                # corrector step
                x = self.adams_moulton_update(order = corrector_order_used, x = x, tau = tau(t), model_prev_list = model_prev_list, t_prev_list = t_prev_list, noise = noise, t = t)
            ...
            for step in range(max(predictor_order, corrector_order - 1), steps + 1):
                ...
                # predictor step
                if skip_final_step and step == steps and not denoise_to_zero:
                    x_p = self.adams_bashforth_update(order = predictor_order_used, x = x, tau = 0, model_prev_list = model_prev_list, t_prev_list = t_prev_list, noise = noise, t = t)
                else:
                    x_p = self.adams_bashforth_update(order = predictor_order_used, x = x, tau = tau(t), model_prev_list = model_prev_list, t_prev_list = t_prev_list, noise = noise, t = t)
    ...
    def sample(self, mode, x, tau, steps, t_start=None, t_end=None, skip_type='time', skip_order=1, predictor_order=3, corrector_order=4, pc_mode='PEC', return_intermediate=False
    ):
        assert mode in ['few_steps', 'more_steps'], "mode must be either 'few_steps' or 'more_steps'"
        if mode == 'few_steps':
            return self.sample_few_steps(x=x, tau=tau, steps=steps, t_start=t_start, t_end=t_end, skip_type=skip_type, skip_order=skip_order, predictor_order=predictor_order, corrector_order=corrector_order, pc_mode=pc_mode, return_intermediate=return_intermediate)
        else:
            return self.sample_more_steps(x=x, tau=tau, steps=steps, t_start=t_start, t_end=t_end, skip_type=skip_type, skip_order=skip_order, predictor_order=predictor_order, corrector_order=corrector_order, pc_mode=pc_mode, return_intermediate=return_intermediate)
